...
Используются **HttpOnly cookies** для безопасного хранения JWT токена. После успешного входа сервер устанавливает cookie, которая автоматически отправляется браузером при каждом запросе. Токен недоступен для JavaScript, что защищает от XSS-атак.
...
| POST | /api/calculate/surface | Расчёт 3D-поверхности характеристик |

Пример запроса /api/calculate:
{
  "w": 0.25,
  "h": 0.01,
  "l": 9.5,
...
Пример ответа:
{
  ...
  "viscosity": 784.8,
  "profile": [
    {"x": 0, "temperature": 145, "viscosity": 8526.1},
